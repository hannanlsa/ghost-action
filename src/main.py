...
def setup_logging():
    ...
    action_log = os.path.join(LOGS_DIR, "action.log")
    ah = logging.handlers.RotatingFileHandler(action_log, maxBytes=5*1024*1024, backupCount=5, encoding="utf-8")
    ah.setFormatter(formatter)
    ah.setLevel(logging.DEBUG)
    action_logger = logging.getLogger("action")
    action_logger.addHandler(ah)
    action_logger.propagate = True

    sync_log = os.path.join(LOGS_DIR, "sync.log")
    sh = logging.handlers.RotatingFileHandler(sync_log, maxBytes=5*1024*1024, backupCount=5, encoding="utf-8")
    sh.setFormatter(formatter)
    sh.setLevel(logging.DEBUG)
    sync_logger = logging.getLogger("sync")
    sync_logger.addHandler(sh)
